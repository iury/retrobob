...
pub const ExHiROM = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, cartridge: *Cartridge, openbus: *u8) !*@This() {
        std.debug.print("Mapper: ExHiROM\n", .{});
        const instance = try allocator.create(ExHiROM);
        instance.* = .{
            .allocator = allocator,
    ...
    fn convertAddress(self: *@This(), address: u24) ?usize {
        ...
        return switch (@as(u8, @truncate(address >> 16))) {
            0x00...0x3f => switch (@as(u16, @truncate(address & 0xffff))) {
                ...
                // SRAM
                0x6000...0x7fff => ((address >> 16) * 0x2000) + (address & 0x1fff),
                // mirror ROM
                0x8000...0xffff => (((address >> 16) + 0x40) * 0x10000) + (address & 0xffff),
            },
            // ROM
            0x40...0x7d => ((address >> 16) * 0x10000) + (address & 0xffff),
            0x80...0xbf => switch (@as(u16, @truncate(address & 0xffff))) {
                // IO
                0x0000...0x5fff => address & 0xffff,
                // SRAM
                0x6000...0x7fff => (((address >> 16) - 0x80) * 0x2000) + (address & 0x1fff),
                // mirror ROM
